...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        # self.add_flow(datapath, 3, match1, actions1, meter=True)

        # match2 = parser.OFPMatch(eth_type = 0x0806, arp_op = 2)
        # actions2 = [parser.OFPActionOutput(ofproto.OFPP_NORMAL)]
        # self.add_flow(datapath, 3, match2, actions2, meter=True)
    ...
    def _packet_in_handler(self, ev):
        ...
        if pkt_dhcp:
            self.logger.info('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh')

        self.logger.info('eeeeeeeeeeeeeeeeee')
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                self.add_flow(datapath, 1, match, actions, msg.buffer_id, meter=None)
                return
            else:
                self.add_flow(datapath, 1, match, actions, meter=None)
        data = None
        if msg.buffer_id == ofproto.OFP_NO_BUFFER:
            data = msg.data
